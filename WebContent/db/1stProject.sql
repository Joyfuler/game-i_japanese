...
DROP TABLE MEMBER;
SELECT * FROM MEMBER;
SELECT B.*, COUNT(BC.*) FROM BOARD B, BOARD_COMMENT BC WHERE B.BNO=BC.BNO AND B.BNO = 9;
...
-- 게시글 신고용 테이블 생성
CREATE SEQUENCE REPORT_SEQ START WITH 1 INCREMENT BY 1 MAXVALUE 99999 NOCACHE NOCYCLE;
CREATE TABLE REPORT(
    RNO NUMBER(5) PRIMARY KEY,
    RREASON NUMBER(1) NOT NULL,
    MID VARCHAR2(120) NOT NULL,
    BNO NUMBER(8),
...
-- 게시글 신고하기    
INSERT INTO REPORT (RNO, RREASON, MID, BNO)
VALUES (REPORT_SEQ.NEXTVAL, '1', 'jjj', '21');
COMMIT;

-- 신고글 리스트 출력
SELECT BOARD.*, MEMBER.MID, MEMBER.MNICKNAME, REPORT.RNO, REPORT.RREASON, REPORT.MID AS REPORTERMID, REPORT.REPORTDATE FROM BOARD,MEMBER,REPORT WHERE BOARD.BNO = REPORT.BNO AND MEMBER.MID = REPORT.MID ORDER BY REPORTDATE DESC; 
COMMIT;
SELECT * FROM REPORT, BOARD WHERE REPORT.BNO = BOARD.BNO AND BOARD.BNO = 9;
    
-- 신고시 자식데이터 삭제    
DELETE FROM REPORT WHERE BNO = 21;
SELECT * FROM MEMBER;
...
INSERT INTO MEMBER (MID, MNICKNAME, MPW, MEMAIL, MPHONE, MPHOTO, MQUEST, MANSWER)
    VALUES ('nonmember', '비회원', '010101', 'dd@ee.cowm', '010-2525-6666', 'unnamed.png', '3', '몰라');                
...
SELECT * FROM (
    ...
    FROM MEMBER M, BOARD B, GAME G, BOARD_COMMENT BC WHERE B.BNO = BC.BNO AND B.GID = G.GID AND G.GID = 'genshin'
