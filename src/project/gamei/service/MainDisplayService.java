...
public class MainDisplayService implements Service {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {		
		...
		// 메인 페이지 상단 검색창에 출력되는 소개문구 및 검색어를 전달. 검색어는 1~4 중 랜덤으로 출력된다.
		SearchWordDao sDao = SearchWordDao.getInstance();
		int sid = (int) (Math.random() * 4) + 1;
		request.setAttribute("searchWord", sDao.randomSearchWord(sid));
		
		// 메인 페이지 상단 article 영역 사진과 a태그 정보를 전달.		
		ArticleDao aDao = ArticleDao.getInstance();
		request.setAttribute("articles", aDao.getArticle());
		
		// 메인 페이지 중앙 body부분의 페이지 당 5개의 게임 리스트를 전달. pageNum이 없다면 1페이지로 출력.
